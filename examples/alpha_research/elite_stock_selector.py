...
class EliteStockSelector:
    ...
    def select_elite(self, candidates):
        ...
        if len(qualified) < self.target_count:
            print(f"⚠️ 仅 {len(qualified)} 只股票满足阈值 (>=70 分)，降低标准...")
            # 降低阈值到 65
        ...
        for i, stock in enumerate(self.selected_stocks, 1):
            ...
            print(f"\n  {i}. {stock['symbol']} {name}")
            print(f"     行业：{comp.get('sector', 'N/A')}")
            print(f"     综合评分：{stock['final_score']:.1f}")
            print(f"       - 基本面：{comp['fundamental_score']:.0f} (40%)")
            print(f"       - 消息面：{comp['news_score']:.0f} (25%)")
            print(f"       - 时政面：{comp['policy_score']:.0f} (20%)")
            print(f"       - 国际形势：{comp['geopolitics_score']:.0f} (15%)")
            print(f"     策略：{'+'.join(stock['strategies'])}")
            print(f"     建议：{comp['recommendation'].get('icon', '')} {comp['recommendation'].get('action', '')}")
