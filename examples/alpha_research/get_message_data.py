...
def get_stock_news(symbol: str = "000001", start_date: str = "20240101", end_date: str = None):
    ...
    try:
        ...
        if df is not None and not df.empty:
            logger.info(f"✓ 成功：获取到 {len(df)} 条新闻")
            logger.info(f"列名：{df.columns.tolist()}")
            return df
        else:
            logger.info(f"✗ 未获取到新闻")
            return None
...
def get_stock_announcement(symbol: str = "000001", start_date: str = "20240101", end_date: str = None):
    ...
    try:
        ...
        logger.info(f"✓ 成功：获取到 {len(df)} 条公告")
        return df
...
def get_stock_info(symbol: str = "000001"):
    ...
    try:
        ...
        if df is not None and not df.empty:
            logger.info(f"✓ 成功：获取到基本信息")
            logger.info(df)
            return df
        else:
            logger.info(f"✗ 未获取到信息")
            return None
...
def get_stock_billboard(start_date: str = None, end_date: str = None):
    ...
    try:
        ...
        if df is not None and not df.empty:
            logger.info(f"✓ 成功：获取到 {len(df)} 条龙虎榜数据")
            logger.info(f"列名：{df.columns.tolist()}")
            return df
        else:
...
def get_institution_survey(symbol: str = None, start_date: str = None, end_date: str = None):
    ...
    try:
        ...
        if df is not None and not df.empty:
            logger.info(f"✓ 成功：获取到 {len(df)} 条调研数据")
            return df
        else:
            logger.info(f"✗ 未获取到数据")
            return None
...
def get_research_report(symbol: str = None, start_date: str = None, end_date: str = None):
    ...
    try:
        ...
        if df is not None and not df.empty:
            logger.info(f"✓ 成功：获取到 {len(df)} 条研报")
            return df
        else:
            logger.info(f"✗ 未获取到数据")
            return None
...
def get_main_hold(symbol: str = None, quarter: str = None):
    ...
    try:
        ...
        if df is not None and not df.empty:
            logger.info(f"✓ 成功：获取到 {len(df)} 条持仓数据")
            return df
        else:
            logger.info(f"✗ 未获取到数据")
            return None
...
if __name__ == "__main__":
    ...
    if news_df is not None and len(news_df) > 0:
        logger.info("\n最新新闻:")
        logger.info(news_df.head(3).to_string())
    
    # 2. 获取个股基本信息
    logger.info("\n" + "=" * 70)
    logger.info("[2] 个股基本信息")
    logger.info("=" * 70)
    ...
    if billboard_df is not None and len(billboard_df) > 0:
        logger.info("\n最新龙虎榜:")
        logger.info(billboard_df.head(3).to_string())
    ...
    if survey_df is not None and len(survey_df) > 0:
        logger.info("\n最新调研:")
        logger.info(survey_df.head(3).to_string())
    ...
    if report_df is not None and len(report_df) > 0:
        logger.info("\n最新研报:")
        logger.info(report_df.head(3).to_string())
    ...
    if hold_df is not None and len(hold_df) > 0:
        logger.info("\n主力持仓:")
        logger.info(hold_df.head(3).to_string())
